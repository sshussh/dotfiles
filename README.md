...
| Package | Deploys |
| --- | --- |
| `gnome-desktop` | GTK overrides, desktop associations, user directories, monitor layout, OpenRGB autostart, pavucontrol, and the Matugen user service |
...
profiles, Discord, mail clients, Codex state, dconf, connection databases,
credentials, history files, caches, logs, backups, generated menus, nested Git
metadata, and machine-session data. No Btrfs Assistant user configuration was
present to include; `connections.db` was excluded because it is an opaque
database and may contain remote/session information.
